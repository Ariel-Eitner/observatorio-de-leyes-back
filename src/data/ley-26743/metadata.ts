import type { Law } from '../../common/types/law.types';

type LawBase = Omit<Law, 'articles' | 'sections'>;

export const LEY_26743_METADATA: LawBase = {
  id: 'ley-26743',
  number: '26743',
  title: 'Identidad de Género',
  commonName: 'Ley de Identidad de Género',
  summary:
    'Reconoce el derecho de toda persona a la identidad de género autopercibida y establece el procedimiento para la rectificación registral del sexo y el cambio de nombre de pila sin necesidad de cirugías, tratamientos hormonales ni autorización judicial. Es considerada una de las leyes de identidad de género más progresistas del mundo al momento de su sanción.',
  year: 2012,
  sanctionDate: '2012-05-09',
  promulgationDate: '2012-05-23',
  publicationDate: '2012-05-24',
  effectiveDate: null,
  derogatedDate: null,
  boNumber: '32404',
  status: 'VIGENTE',
  jurisdiction: 'NACIONAL',
  normType: 'LEY',
  issuingBody: 'Honorable Congreso de la Nación Argentina',
  fullText: null,
  sourceUrl: 'https://servicios.infoleg.gob.ar/infolegInternet/anexos/195000-199999/197860/norma.htm',
  articleCount: 15,
  topics: [
    'Identidad de género',
    'Derechos humanos',
    'Registro civil',
    'Salud integral',
    'Diversidad sexual',
  ],
  keywords: [
    'identidad de género',
    'autopercepción',
    'rectificación registral',
    'cambio de nombre',
    'DNI',
    'diversidad sexual',
    'transgénero',
    'no binario',
    'trato digno',
    'salud integral',
    'objeción de conciencia',
    'Plan Médico Obligatorio',
  ],
  relatedNorms: [
    'Ley 26.485 (Protección integral contra la violencia contra las mujeres)',
    'Ley 18.248 (Nombre de las personas naturales)',
    'Ley 17.132 (Ejercicio de la medicina — artículo 19 inc. 4 derogado)',
    'Ley 26.061 (Protección integral de los derechos de NNyA)',
    'Convención sobre los Derechos del Niño',
  ],
  executiveSummary:
    'La Ley de Identidad de Género reconoce el derecho de toda persona a ser reconocida según su identidad autopercibida, sin importar el sexo asignado al nacer. Permite cambiar el nombre y el sexo en el DNI y partida de nacimiento con un trámite gratuito, personal y sin intermediarios, sin requerir cirugías, tratamientos hormonales ni diagnósticos psiquiátricos. Las personas mayores de 18 pueden acceder directamente; los menores de 18 lo hacen a través de sus representantes legales o, si estos se niegan, por vía judicial sumaria. El sistema de salud (público, privado y obras sociales) debe garantizar intervenciones quirúrgicas y tratamientos hormonales con solo el consentimiento informado del adulto. La ley deroga el inciso 4° del artículo 19 de la Ley 17.132 que prohibía las cirugías de reasignación.',
  objective:
    'Garantizar el derecho humano a la identidad de género autopercibida, eliminando los requisitos médicos y judiciales que históricamente impedían o dificultaban el reconocimiento de la identidad de personas trans y no binarias.',
  problemItSolves:
    'Antes de esta ley, para cambiar el sexo en el documento era necesario someterse a cirugía genital y obtener autorización judicial, lo que implicaba medicalizaciones forzadas e intervenciones judiciales sobre el cuerpo. Muchas personas trans vivían con documentos que no coincidían con su identidad, lo que generaba discriminación permanente en el trabajo, la salud y los espacios públicos.',
  practicalImpact:
    'Desde 2012, más de 16.000 personas han rectificado su género en el Registro Nacional. Argentina fue pionera mundial en eliminar los requisitos médicos y judiciales para el cambio registral. El "trato digno" (art. 12) obliga a instituciones públicas y privadas a usar el nombre autopercibido aunque el DNI no esté actualizado. La ley fue modificada de hecho por la Res. 1/2018 del RENAPER para incorporar la opción de género no binario ("X") en el DNI.',
  affectedSubjects: [
    'Personas trans, no binarias y con identidad de género diversa',
    'Registro Nacional de las Personas (RENAPER)',
    'Efectores del sistema público de salud',
    'Obras sociales y prepagas (PMO)',
    'Instituciones públicas y privadas (obligación de trato digno)',
    'Padres/madres de menores trans',
    'Poder Judicial (resolución en 60 días para cirugías de menores)',
  ],
  createdAt: '2012-05-24T00:00:00.000Z',
  updatedAt: '2012-05-24T00:00:00.000Z',
  relations: [],
  amendments: [],
  annexes: [],
  segments: [],
  metadata: {
    id: 'meta-26743',
    lawId: 'ley-26743',
    mainTopic: 'Derecho a la identidad de género autopercibida',
    subtopics: [
      'Rectificación registral del sexo',
      'Cambio de nombre sin autorización médica ni judicial',
      'Acceso a salud integral trans',
      'Trato digno en instituciones',
      'Identidad de género en menores',
    ],
    relatedLaws: [
      'Ley 18.248 — Nombre de las personas naturales (1969)',
      'Ley 17.132 — Ejercicio de la medicina (art. 19 inc. 4 derogado)',
      'Ley 26.061 — Protección integral de NNyA (2005)',
    ],
    regulations: ['Decreto 1007/2012 (reglamentario)', 'Resolución RENAPER 1/2018 (género no binario)'],
    modifyingNorms: ['Ley 27.636 (Diana Sacayán — cupo laboral trans, 2021)'],
    derogatingNorms: [],
    jurisprudence: [
      'CSJN — "Comisión Interamericana de Derechos Humanos c/ Argentina" — Argentina fue observada por la CIDH por demoras en implementación para menores',
      'Juzgados de familia — múltiples fallos admitieron el cambio de género para menores sin consentimiento de ambos progenitores, aplicando el interés superior del niño',
    ],
    doctrine: [],
    obligations: [
      'Tramitar rectificación registral sin exigir cirugía, hormonal ni diagnóstico (RENAPER)',
      'Usar el nombre autopercibido en todos los procedimientos (instituciones públicas y privadas)',
      'Garantizar acceso a tratamientos hormonales y quirúrgicos con solo consentimiento informado (sistema de salud)',
      'Expedirse en 60 días para consentimiento de cirugías de menores (Poder Judicial)',
      'Mantener confidencialidad del acta de nacimiento original (RENAPER)',
    ],
    rights: [
      'Derecho al reconocimiento de la identidad de género autopercibida',
      'Derecho a ser tratado con el nombre elegido en cualquier institución',
      'Derecho a acceder a intervenciones quirúrgicas y hormonales con solo consentimiento informado (mayores)',
      'Derecho a la confidencialidad del historial registral',
      'Derecho de menores a que se respete su identidad con acompañamiento legal',
    ],
    sanctions: [],
    useCases: [
      'Una mujer trans de 25 años quiere cambiar su nombre y sexo en el DNI → va al RENAPER, presenta solicitud gratuita sin intermediarios, obtiene nuevo DNI. No necesita cirugía ni sentencia judicial.',
      'Una persona no binaria quiere que en su DNI figure "X" en lugar de M o F → desde 2018 puede solicitarlo al RENAPER por Resolución 1/2018 que extiende la ley.',
      'Un empleador llama a un trabajador trans por el nombre de su viejo DNI → viola el art. 12 de la ley; puede ser denunciado por discriminación.',
      'Un adolescente de 16 años quiere cambiar su género → sus representantes legales pueden solicitarlo; si se niegan, puede acudir a la justicia que debe resolver en vía sumaria.',
    ],
    faq: [
      {
        question: '¿Hay que operarse para cambiar el género en el DNI?',
        answer:
          'No. La ley expresamente prohíbe exigir "intervención quirúrgica por reasignación genital total o parcial" como requisito. Solo se necesita presentar la solicitud ante el RENAPER manifestando la identidad autopercibida.',
      },
      {
        question: '¿El nuevo DNI muestra que hubo un cambio de género?',
        answer:
          'No. Se prohíbe hacer cualquier referencia a la ley en el nuevo DNI o partida de nacimiento. El acta de nacimiento original es confidencial y solo puede ser consultada por el titular o con orden judicial.',
      },
      {
        question: '¿La obra social debe cubrir las cirugías de reasignación?',
        answer:
          'Sí. El artículo 11 incluye las prestaciones de salud trans en el Plan Médico Obligatorio (PMO). El sistema público, obras sociales y prepagas deben cubrirlas sin necesidad de autorización judicial o administrativa para los adultos.',
      },
    ],
    createdAt: '2012-05-24T00:00:00.000Z',
    updatedAt: '2012-05-24T00:00:00.000Z',
  },
};
