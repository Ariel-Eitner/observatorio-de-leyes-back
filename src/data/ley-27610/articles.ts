import type { Article } from '../../common/types/law.types';

export const ARTICLES_27610: Article[] = [
  {
    id: 'art-27610-1',
    lawId: 'ley-27610',
    number: '1',
    title: 'Objeto',
    text:
      'La presente ley tiene por objeto regular el acceso a la interrupción voluntaria del embarazo y a la atención postaborto, en cumplimiento de los compromisos asumidos por el Estado argentino en materia de salud pública y derechos humanos de las mujeres y de personas con otras identidades de género con capacidad de gestar y a fin de contribuir a la reducción de la morbilidad y mortalidad prevenible.',    plainLanguageExplanation:
      'Esta ley regula el derecho a interrumpir voluntariamente el embarazo y a recibir atención médica postaborto. Su fundamento son los compromisos de Argentina en salud pública y derechos humanos, y el objetivo de reducir las muertes y enfermedades evitables derivadas de abortos clandestinos.',
    practicalEffects: [
      'Convierte el acceso al aborto en un derecho regulado por ley, no una tolerancia o excepción puntual',
      'Incluye expresamente a personas con identidades de género no binarias con capacidad de gestar',
      'Habilita la atención postaborto incluso cuando el aborto no estuvo encuadrado en los casos legales',
    ],
    examples: [
      'Una persona gestante que interrumpió su embarazo fuera de los supuestos de la ley tiene igualmente derecho a recibir atención postaborto en el sistema de salud',
    ],
    relatedArticles: ['art-27610-2', 'art-27610-4'],
    jurisprudence: [],
    regulations: ['Decreto 14/2021 (reglamentario y de observación parcial)'],
    keywords: ['objeto', 'IVE', 'interrupción voluntaria', 'postaborto', 'salud pública', 'derechos humanos'],
    order: 1,
    segments: [
      {
        id: 'seg-27610-1-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-1',
        articleNumber: '1',
        segmentType: 'PARAGRAPH',
        text:
          'La presente ley tiene por objeto regular el acceso a la interrupción voluntaria del embarazo y a la atención postaborto...',
        plainExplanation:
          'La ley regula el aborto voluntario y la atención médica después del aborto para reducir muertes evitables.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-2',
    lawId: 'ley-27610',
    number: '2',
    title: 'Derechos',
    text:
      'Las mujeres y personas con otras identidades de género con capacidad de gestar tienen derecho a: a) Decidir la interrupción del embarazo de conformidad con lo establecido en la presente ley; b) Requerir y acceder a la atención de la interrupción del embarazo en los servicios del sistema de salud, de conformidad con lo establecido en la presente ley; c) Requerir y recibir atención postaborto en los servicios del sistema de salud, sin perjuicio de que la decisión de abortar hubiera sido contraria a los casos legalmente habilitados de conformidad con la presente ley; d) Prevenir los embarazos no intencionales mediante el acceso a información, educación sexual integral y a métodos anticonceptivos eficaces.',    plainLanguageExplanation:
      'Este artículo enumera los cuatro derechos concretos: decidir interrumpir el embarazo, acceder al sistema de salud para hacerlo, recibir atención postaborto (incluso si el aborto fue fuera de los plazos legales), y acceder a anticoncepción e información para evitar embarazos no deseados.',
    practicalEffects: [
      'El derecho al aborto incluye el acceso efectivo al sistema de salud, no solo la no persecución penal',
      'La atención postaborto es un derecho autónomo: no puede negarse aunque el aborto haya sido "ilegal"',
      'La anticoncepción y la ESI son parte del mismo sistema de derechos reproductivos',
    ],
    examples: [
      'Una mujer que abortó por medios propios fuera del plazo legal tiene derecho a atención médica postaborto sin consecuencias penales para el personal de salud que la atiende',
    ],
    relatedArticles: ['art-27610-1', 'art-27610-4', 'art-27610-5'],
    jurisprudence: [],
    regulations: [],
    keywords: ['derechos', 'decidir', 'acceder', 'postaborto', 'anticoncepción', 'ESI'],
    order: 2,
    segments: [
      {
        id: 'seg-27610-2-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-2',
        articleNumber: '2',
        segmentType: 'PARAGRAPH',
        text:
          'Las mujeres y personas con otras identidades de género con capacidad de gestar tienen derecho a: a) Decidir la interrupción del embarazo...',
        plainExplanation:
          'Cuatro derechos: decidir, acceder al sistema de salud, recibir atención postaborto y prevenir embarazos no deseados.',
        practicalExample:
          'Incluso si una mujer abortó por sus propios medios, el hospital no puede negarle atención postaborto.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-3',
    lawId: 'ley-27610',
    number: '3',
    title: 'Marco normativo constitucional',
    text:
      'Las disposiciones de la presente ley se enmarcan en el artículo 75, inciso 22, de la Constitución Nacional, los tratados de derechos humanos ratificados por la República Argentina, en especial la Declaración Universal de Derechos Humanos, la Convención Americana sobre Derechos Humanos, la Convención sobre la Eliminación de todas las Formas de Discriminación contra la Mujer (CEDAW por sus siglas en inglés) y su Protocolo Facultativo, el Pacto Internacional de Derechos Económicos, Sociales y Culturales, el Pacto Internacional de Derechos Civiles y Políticos, la Convención Interamericana para Prevenir, Sancionar y Erradicar la Violencia contra la Mujer "Convención de Belém do Pará", la Convención sobre los Derechos de las Personas con Discapacidad, la Convención sobre los Derechos del Niño y la Convención contra la Tortura y Otros Tratos o Penas Crueles, Inhumanos o Degradantes, en virtud de la protección que otorgan a los derechos sexuales y reproductivos, a la dignidad, a la vida, a la autonomía, a la salud, a la educación, a la integridad, a la diversidad corporal, a la identidad de género, a la diversidad étnico-cultural, a la privacidad, a la libertad de creencias y pensamientos, a la información, a gozar de los beneficios de los avances científicos, a la igualdad real de oportunidades, a la no discriminación y a una vida libre de violencias.',    plainLanguageExplanation:
      'La ley IVE se funda en el artículo 75 inciso 22 de la Constitución Nacional (que incorpora los tratados internacionales de derechos humanos al bloque constitucional) y en una larga lista de convenciones que el artículo enumera explícitamente. Esto significa que el derecho al aborto voluntario se asienta en compromisos internacionales vinculantes para Argentina.',
    practicalEffects: [
      'El derecho a la IVE tiene rango constitucional por vía de los tratados internacionales',
      'Cualquier restricción al acceso es inconstitucional y viola compromisos internacionales de Argentina',
    ],
    examples: [],
    relatedArticles: ['art-27610-1'],
    jurisprudence: [],
    regulations: [],
    keywords: ['marco constitucional', 'tratados internacionales', 'CEDAW', 'Belém do Pará', 'derechos reproductivos'],
    order: 3,
    segments: [
      {
        id: 'seg-27610-3-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-3',
        articleNumber: '3',
        segmentType: 'PARAGRAPH',
        text:
          'Las disposiciones de la presente ley se enmarcan en el artículo 75, inciso 22, de la Constitución Nacional...',
        plainExplanation:
          'La ley se basa en la Constitución y en tratados internacionales de derechos humanos que Argentina firmó y que tienen rango constitucional.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-4',
    lawId: 'ley-27610',
    number: '4',
    title: 'Interrupción voluntaria del embarazo',
    text:
      'Las mujeres y personas con otras identidades de género con capacidad de gestar tienen derecho a decidir y acceder a la interrupción de su embarazo hasta la semana catorce (14), inclusive, del proceso gestacional. Fuera del plazo dispuesto en el párrafo anterior, la persona gestante tiene derecho a decidir y acceder a la interrupción de su embarazo solo en las siguientes situaciones: a) Si el embarazo fuere resultado de una violación, con el requerimiento y la declaración jurada pertinente de la persona gestante, ante el personal de salud interviniente. En los casos de niñas menores de trece (13) años de edad, la declaración jurada no será requerida; b) Si estuviere en peligro la vida o la salud integral de la persona gestante.',    plainLanguageExplanation:
      'Hasta la semana 14 inclusive: aborto voluntario sin necesidad de justificación. Después de la semana 14: solo en dos casos: violación (con declaración jurada; las niñas menores de 13 no la necesitan) o riesgo para la vida o salud integral de la persona gestante (salud integral incluye salud mental).',
    practicalEffects: [
      'Hasta semana 14: derecho irrestricto, sin exigir motivo',
      'Desde semana 15 en adelante: solo violación o riesgo de vida/salud',
      'En caso de violación no se necesita denuncia penal, solo declaración jurada ante el médico',
      'Niñas menores de 13 años: no necesitan ni declaración jurada en caso de violación',
    ],
    examples: [
      'Una mujer de 10 semanas de embarazo puede interrumpirlo sin dar ninguna explicación',
      'Una mujer embarazada de 20 semanas producto de una violación presenta declaración jurada ante el médico y accede al aborto legal',
    ],
    relatedArticles: ['art-27610-2', 'art-27610-5', 'art-27610-10', 'art-27610-16'],
    jurisprudence: [],
    regulations: [],
    keywords: ['semana 14', 'violación', 'salud integral', 'plazo gestacional', 'declaración jurada'],
    order: 4,
    segments: [
      {
        id: 'seg-27610-4-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-4',
        articleNumber: '4',
        segmentType: 'PARAGRAPH',
        text:
          'Las mujeres y personas con otras identidades de género con capacidad de gestar tienen derecho a decidir y acceder a la interrupción de su embarazo hasta la semana catorce (14), inclusive...',
        plainExplanation:
          'Hasta semana 14: aborto libre. Después: solo por violación (con declaración jurada) o riesgo de vida/salud.',
        practicalExample:
          'Una mujer embarazada de 20 semanas por violación lleva una declaración jurada al médico; no necesita denuncia policial.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-5',
    lawId: 'ley-27610',
    number: '5',
    title: 'Derechos en la atención de la salud',
    text:
      'Toda persona gestante tiene derecho a acceder a la interrupción de su embarazo en los servicios del sistema de salud o con su asistencia, en un plazo máximo de diez (10) días corridos desde su requerimiento y en las condiciones que se establecen en la presente ley y en las leyes 26.485, 26.529 y concordantes. El personal de salud debe garantizar las siguientes condiciones mínimas y derechos en la atención del aborto y postaborto: a) Trato digno. El personal de salud debe observar un trato digno, respetando las convicciones personales y morales de la paciente, para erradicar prácticas que perpetúan el ejercicio de violencia contra las mujeres y personas con otras identidades de género con capacidad de gestar; b) Privacidad. Toda actividad médico-asistencial tendiente a obtener y transmitir información y documentación clínica de la paciente debe garantizar la construcción y preservación de un ambiente de confianza entre el personal de salud y las personas que solicitan la atención, y observar el estricto respeto por su intimidad, dignidad humana y autonomía de la voluntad, así como el debido resguardo de la confidencialidad; solo se compartirá información o se incluirá a su familia o a su acompañante con su expresa autorización; d) Autonomía de la voluntad. El personal de salud debe respetar las decisiones de las pacientes respecto al ejercicio de sus derechos reproductivos, las alternativas de tratamiento y su futura salud sexual y reproductiva. Las decisiones de la paciente no deben ser sometidas a juicios derivados de consideraciones personales, religiosas o axiológicas por parte del personal de salud, debiendo prevalecer su libre y autónoma voluntad; e) Acceso a la información. El personal de salud debe mantener una escucha activa y respetuosa de las pacientes para expresar libremente sus necesidades y preferencias. La paciente tiene derecho a recibir la información sobre su salud; el derecho a la información incluye el de no recibir información inadecuada en relación con la solicitada. f) Calidad. El personal de salud debe respetar y garantizar el tratamiento del aborto conforme los alcances y la definición de la Organización Mundial de la Salud.',    plainLanguageExplanation:
      'El sistema de salud tiene un plazo máximo de 10 días corridos para practicar el aborto desde que la persona lo solicita. Además, el personal de salud debe garantizar: trato digno (sin discriminación), privacidad y confidencialidad, autonomía de la voluntad (sin presiones ni juicios morales), acceso a información veraz, y calidad conforme los estándares de la OMS.',
    practicalEffects: [
      'Si pasan 10 días sin que el sistema de salud resuelva, la persona puede exigir judicialmente el acceso inmediato',
      'El médico no puede imponer su opinión moral: debe respetar la decisión de la paciente',
      'La confidencialidad protege a la paciente: el médico no puede compartir información con familiares sin autorización',
    ],
    examples: [
      'Una mujer solicita una IVE el lunes 1; el servicio de salud tiene hasta el jueves 11 para realizarla',
      'Si el médico le dice "pensalo bien" o "es un pecado", viola el derecho al trato digno y a la autonomía',
    ],
    relatedArticles: ['art-27610-4', 'art-27610-10', 'art-27610-11'],
    jurisprudence: [],
    regulations: [],
    keywords: ['plazo 10 días', 'trato digno', 'confidencialidad', 'autonomía', 'calidad OMS'],
    order: 5,
    segments: [
      {
        id: 'seg-27610-5-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-5',
        articleNumber: '5',
        segmentType: 'PARAGRAPH',
        text:
          'Toda persona gestante tiene derecho a acceder a la interrupción de su embarazo en los servicios del sistema de salud o con su asistencia, en un plazo máximo de diez (10) días corridos desde su requerimiento...',
        plainExplanation:
          'El sistema de salud tiene 10 días máximos para practicar el aborto. Debe garantizar trato digno, privacidad, autonomía, información y calidad.',
        practicalExample:
          'Si solicito la IVE el lunes 1, el hospital tiene hasta el jueves 11 para realizarla.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-6',
    lawId: 'ley-27610',
    number: '6',
    title: 'Información y tratamiento del aborto y de la salud sexual y reproductiva',
    text:
      'Realizada la solicitud de interrupción voluntaria del embarazo de conformidad con el artículo 4º, el establecimiento de salud pondrá a disposición de las personas gestantes que así lo requieran, en el marco del Programa Nacional de Salud Sexual y Procreación Responsable, ley 25.673, lo siguiente: a) Información sobre el procedimiento que se llevará a cabo y los cuidados posteriores necesarios, siguiendo los criterios del artículo anterior; b) Atención integral de su salud a lo largo de todo el proceso; c) Acompañamiento en el cuidado de la salud e información adecuada y accesible a las necesidades de cada persona, científica, actualizada sobre los distintos métodos anticonceptivos disponibles, así como la provisión de los métodos anticonceptivos previstos en el Programa Médico Obligatorio (PMO) y en la ley 25.673 o la normativa que en el futuro la reemplace. Estos servicios no son obligatorios para la paciente ni condición para la realización de la práctica.',    plainLanguageExplanation:
      'Tras la solicitud de IVE, el efector debe ofrecer (no imponer): información sobre el procedimiento y cuidados posteriores, acompañamiento de salud integral, y asesoramiento en anticoncepción con provisión del método elegido según el PMO. La paciente puede rechazar cualquiera de estos servicios y el aborto igual se realiza.',
    practicalEffects: [
      'El médico debe informar sobre el procedimiento, pero no puede condicionarlo a aceptar asesoramiento anticonceptivo',
      'Los anticonceptivos del PMO son gratuitos y deben proveerse si la paciente los solicita',
    ],
    examples: [
      'Una mujer que solicita una IVE tiene derecho a recibir un implante anticonceptivo del PMO sin costo después del procedimiento, si lo desea',
    ],
    relatedArticles: ['art-27610-5', 'art-27610-7'],
    jurisprudence: [],
    regulations: [],
    keywords: ['información', 'anticoncepción', 'PMO', 'acompañamiento', 'no obligatorio'],
    order: 6,
    segments: [
      {
        id: 'seg-27610-6-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-6',
        articleNumber: '6',
        segmentType: 'PARAGRAPH',
        text:
          'Estos servicios no son obligatorios para la paciente ni condición para la realización de la práctica.',
        plainExplanation:
          'El hospital debe ofrecer información y anticoncepción, pero la paciente puede negarse y el aborto se realiza igual.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-7',
    lawId: 'ley-27610',
    number: '7',
    title: 'Consentimiento informado',
    text:
      'Previo a la realización de la interrupción voluntaria del embarazo se requiere el consentimiento informado de la persona gestante expresado por escrito, de conformidad con lo previsto en la ley 26.529 y concordantes y en el artículo 59 del Código Civil y Comercial de la Nación. Nadie puede ser sustituido en el ejercicio personal de este derecho.',    plainLanguageExplanation:
      'Antes de practicarse el aborto, la persona debe firmar un consentimiento informado por escrito. Nadie puede firmar por ella ni sustituirla: es un derecho personalísimo que solo puede ejercer la persona gestante.',
    practicalEffects: [
      'La firma del consentimiento informado es condición previa al procedimiento',
      'Un marido, padre o juez no puede dar el consentimiento en lugar de la persona gestante',
    ],
    examples: [
      'El marido de una mujer no puede impedir el aborto alegando que él no consiente; el derecho es personal de la persona gestante',
    ],
    relatedArticles: ['art-27610-5', 'art-27610-8', 'art-27610-9'],
    jurisprudence: [],
    regulations: [],
    keywords: ['consentimiento informado', 'escrito', 'personalísimo', 'nadie puede sustituir'],
    order: 7,
    segments: [
      {
        id: 'seg-27610-7-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-7',
        articleNumber: '7',
        segmentType: 'PARAGRAPH',
        text:
          'Nadie puede ser sustituido en el ejercicio personal de este derecho.',
        plainExplanation:
          'El consentimiento informado es personal e intransferible. Solo la persona gestante puede darlo.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-8',
    lawId: 'ley-27610',
    number: '8',
    title: 'Personas menores de edad',
    text:
      'En el marco de lo establecido en la Convención sobre los Derechos del Niño, la ley 26.061, el artículo 7º del anexo I del decreto 415/06, el artículo 26 del Código Civil y Comercial de la Nación y la resolución 65/15 del Ministerio de Salud de la Nación, la solicitud de la interrupción voluntaria del embarazo deberá ser efectuada de la siguiente manera: a) Las personas mayores de dieciséis (16) años de edad tienen plena capacidad por sí para prestar su consentimiento a fin de ejercer los derechos que otorga la presente ley; b) En los casos de personas menores de dieciséis (16) años de edad, se requerirá su consentimiento informado en los términos del artículo anterior y se procederá conforme lo dispuesto en el artículo 26 del Código Civil y Comercial y la resolución 65/15 del Ministerio de Salud de la Nación en concordancia con la Convención de los Derechos del Niño, la ley 26.061, el artículo 7º del anexo I del decreto reglamentario 415/06 y el decreto reglamentario 1.282/03 de la ley 25.673.',    plainLanguageExplanation:
      'Desde los 16 años: capacidad plena para dar el consentimiento sin necesidad de autorización de padres o tutores. Menores de 16: se aplica la doctrina de la capacidad progresiva del art. 26 CCyCN —el sistema de salud evalúa si la persona puede consentir por sí misma según su madurez.',
    practicalEffects: [
      'Una chica de 16 o más años puede solicitar y consentir la IVE sin que sus padres estén involucrados',
      'Una chica de 14 años puede ejercer el derecho aplicando la capacidad progresiva del CCyCN',
    ],
    examples: [
      'Una adolescente de 17 años puede interrumpir su embarazo sin autorización de sus padres',
    ],
    relatedArticles: ['art-27610-7', 'art-27610-9'],
    jurisprudence: [],
    regulations: [],
    keywords: ['menores de edad', 'capacidad progresiva', '16 años', 'consentimiento'],
    order: 8,
    segments: [
      {
        id: 'seg-27610-8-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-8',
        articleNumber: '8',
        segmentType: 'PARAGRAPH',
        text:
          'a) Las personas mayores de dieciséis (16) años de edad tienen plena capacidad por sí para prestar su consentimiento...',
        plainExplanation:
          'Desde los 16: consentimiento pleno sin padres. Menores de 16: capacidad progresiva del CCyCN.',
        practicalExample:
          'Una adolescente de 17 años puede interrumpir su embarazo sin que sus padres lo autoricen.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-9',
    lawId: 'ley-27610',
    number: '9',
    title: 'Personas con capacidad restringida',
    text:
      'Si se tratare de una persona con capacidad restringida por sentencia judicial y la restricción no tuviere relación con el ejercicio de los derechos que otorga la presente ley, podrá prestar su consentimiento informado sin ningún impedimento ni necesidad de autorización previa alguna y, si lo deseare, con la asistencia del sistema de apoyo previsto en el artículo 43 del Código Civil y Comercial de la Nación. Las personas que actúan como sistema de apoyo no representan ni sustituyen a la persona con discapacidad en el ejercicio de sus derechos y, por tanto, es necesario que el diseño del sistema de apoyo incorpore salvaguardas adecuadas para que no existan abusos y las decisiones sean tomadas por la titular del derecho.',    plainLanguageExplanation:
      'Si una persona tiene sentencia de capacidad restringida pero dicha restricción no se vincula con sus derechos reproductivos, puede consentir sola la IVE. El sistema de apoyo la asiste si lo pide, pero no sustituye su decisión.',
    practicalEffects: [
      'Una persona con discapacidad mental cuya sentencia no le prohíbe específicamente ejercer derechos reproductivos puede decidir sola',
      'El curador o apoyos no pueden vetar la decisión',
    ],
    examples: [],
    relatedArticles: ['art-27610-7', 'art-27610-8'],
    jurisprudence: [],
    regulations: [],
    keywords: ['capacidad restringida', 'discapacidad', 'sistema de apoyo', 'autonomía'],
    order: 9,
    segments: [
      {
        id: 'seg-27610-9-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-9',
        articleNumber: '9',
        segmentType: 'PARAGRAPH',
        text:
          'Las personas que actúan como sistema de apoyo no representan ni sustituyen a la persona con discapacidad en el ejercicio de sus derechos...',
        plainExplanation:
          'Los apoyos ayudan pero no deciden. La persona con discapacidad sigue siendo quien toma la decisión.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-10',
    lawId: 'ley-27610',
    number: '10',
    title: 'Objeción de conciencia',
    text:
      'El o la profesional de salud que deba intervenir de manera directa en la interrupción del embarazo tiene derecho a ejercer la objeción de conciencia. A los fines del ejercicio de la misma, deberá: a) Mantener su decisión en todos los ámbitos, público, privado o de la seguridad social, en los que ejerza su profesión; b) Derivar de buena fe a la paciente para que sea atendida por otro u otra profesional en forma temporánea y oportuna, sin dilaciones; c) Cumplir con el resto de sus deberes profesionales y obligaciones jurídicas. El personal de salud no podrá negarse a la realización de la interrupción del embarazo en caso de que la vida o salud de la persona gestante esté en peligro y requiera atención inmediata e impostergable. No se podrá alegar objeción de conciencia para negarse a prestar atención sanitaria postaborto. El incumplimiento de las obligaciones establecidas en el presente artículo dará lugar a las sanciones disciplinarias, administrativas, penales y civiles, según corresponda.',    plainLanguageExplanation:
      'El profesional de salud puede negarse a practicar el aborto por razones de conciencia, pero con tres condiciones: (1) mantener esa decisión en todos los ámbitos donde trabaje (no puede objetar en el público y no en el privado), (2) derivar a la paciente rápidamente sin dilaciones, y (3) cumplir sus otras obligaciones. NO puede objetar si hay riesgo de vida o salud inmediato, ni puede negarse a dar atención postaborto.',
    practicalEffects: [
      'La objeción de conciencia es individual, no institucional',
      'El objetor debe derivar: no puede dejar a la paciente sin atención',
      'En emergencia: no hay objeción posible',
      'El incumplimiento genera sanciones penales, civiles y disciplinarias',
    ],
    examples: [
      'Un médico objetor de conciencia recibe a una mujer con 8 semanas de embarazo: debe derivarla inmediatamente a otro médico que sí practique la IVE',
      'Un médico no puede negarse a atender a una mujer con complicaciones postaborto alegando objeción',
    ],
    relatedArticles: ['art-27610-11', 'art-27610-4'],
    jurisprudence: [],
    regulations: [],
    keywords: ['objeción de conciencia', 'derivar', 'emergencia', 'postaborto', 'sanciones'],
    order: 10,
    segments: [
      {
        id: 'seg-27610-10-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-10',
        articleNumber: '10',
        segmentType: 'PARAGRAPH',
        text:
          'El o la profesional de salud que deba intervenir de manera directa en la interrupción del embarazo tiene derecho a ejercer la objeción de conciencia...',
        plainExplanation:
          'El médico puede negarse por razones de conciencia, pero debe derivar de inmediato y no puede objetar en emergencias ni para atención postaborto.',
        practicalExample:
          'Un médico objetor debe derivar a la paciente sin dilaciones; si hay riesgo de vida, no puede negarse.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-11',
    lawId: 'ley-27610',
    number: '11',
    title: 'Objeción de conciencia. Obligaciones de los establecimientos de salud',
    text:
      'Aquellos efectores de salud del subsector privado o de la seguridad social que no cuenten con profesionales para realizar la interrupción del embarazo a causa del ejercicio del derecho de objeción de conciencia de conformidad con el artículo anterior, deberán prever y disponer la derivación a un efector que realice efectivamente la prestación y que sea de similares características al que la persona solicitante de la prestación consultó. En todos los casos se debe garantizar la realización de la práctica conforme a las previsiones de la presente ley. Las gestiones y costos asociados a la derivación y el traslado de la paciente quedarán a cargo del efector que realice la derivación. Todas las derivaciones contempladas en este artículo deberán facturarse de acuerdo con la cobertura a favor del efector que realice la práctica.',    plainLanguageExplanation:
      'Si una clínica privada o prepaga no tiene médicos disponibles por objeción de conciencia, debe organizar la derivación a otro efector de similares características y pagar los costos del traslado. La práctica siempre debe realizarse y los costos corren por cuenta del efector que deriva.',
    practicalEffects: [
      'Una clínica no puede escudarse en que "todos los médicos son objetores" para no derivar',
      'El costo del traslado lo paga la clínica derivante, no la paciente',
    ],
    examples: [
      'Una prepaga cuya clínica no tiene médicos disponibles debe derivar a su afiliada a otra clínica equivalente y hacerse cargo del traslado',
    ],
    relatedArticles: ['art-27610-10', 'art-27610-12'],
    jurisprudence: [],
    regulations: [],
    keywords: ['establecimientos de salud', 'derivación', 'costos traslado', 'objeción institucional'],
    order: 11,
    segments: [
      {
        id: 'seg-27610-11-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-11',
        articleNumber: '11',
        segmentType: 'PARAGRAPH',
        text:
          'Las gestiones y costos asociados a la derivación y el traslado de la paciente quedarán a cargo del efector que realice la derivación.',
        plainExplanation:
          'Si la clínica deriva porque todos sus médicos son objetores, los gastos de traslado corren por cuenta de la clínica, no de la paciente.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-12',
    lawId: 'ley-27610',
    number: '12',
    title: 'Cobertura y calidad de las prestaciones',
    text:
      'El sector público de la salud, las obras sociales enmarcadas en la ley 23.660 y en la ley 23.661, el Instituto Nacional de Servicios Sociales para Jubilados y Pensionados creado por la ley 19.032, las entidades y agentes de salud comprendidos en la ley 26.682, de marco regulatorio de medicina prepaga, las entidades que brinden atención dentro de la reglamentación del decreto 1.993/11, las obras sociales de las fuerzas armadas y de seguridad, las obras sociales del Poder Legislativo y Judicial y las comprendidas en la ley 24.741, de obras sociales universitarias, y todos aquellos agentes y organizaciones que brinden servicios médico-asistenciales a las personas afiliadas o beneficiarias, independientemente de la figura jurídica que posean, deben incorporar la cobertura integral y gratuita de la interrupción voluntaria del embarazo prevista en la presente ley en todas las formas que la Organización Mundial de la Salud recomienda. Estas prestaciones quedan incluidas en el Programa Nacional de Garantía de Calidad de la Atención Médica y en el PMO con cobertura total, junto con las prestaciones de diagnóstico, medicamentos y terapias de apoyo.',    plainLanguageExplanation:
      'La IVE es gratuita y está incluida en el PMO. Todo el sistema de salud (público, obras sociales, prepagas, PAMI) debe cubrirla sin costo para la persona. La cobertura incluye el diagnóstico, los medicamentos (misoprostol, mifepristona) y las terapias de apoyo.',
    practicalEffects: [
      'La prepaga no puede cobrar un copago o débito por la IVE',
      'El misoprostol y la mifepristona están cubiertos al 100% por el PMO',
      'El PAMI también está obligado a cubrir la IVE para sus afiliadas',
    ],
    examples: [
      'Una afiliada de una prepaga que solicita una IVE no paga nada: ni consulta, ni medicamentos, ni internación si fuera necesaria',
    ],
    relatedArticles: ['art-27610-5', 'art-27610-11'],
    jurisprudence: [],
    regulations: [],
    keywords: ['PMO', 'cobertura gratuita', 'obras sociales', 'prepaga', 'PAMI', 'misoprostol'],
    order: 12,
    segments: [
      {
        id: 'seg-27610-12-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-12',
        articleNumber: '12',
        segmentType: 'PARAGRAPH',
        text:
          'Estas prestaciones quedan incluidas en el Programa Nacional de Garantía de Calidad de la Atención Médica y en el PMO con cobertura total...',
        plainExplanation:
          'La IVE está en el PMO con cobertura total. Todo el sistema de salud debe cubrirla gratis.',
        practicalExample:
          'Una afiliada de una prepaga que solicita una IVE no paga nada: ni consulta, ni medicamentos.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-13',
    lawId: 'ley-27610',
    number: '13',
    title: 'Educación sexual integral y salud sexual y reproductiva',
    text:
      'El Estado nacional, las provincias, la Ciudad Autónoma de Buenos Aires y los municipios tienen la responsabilidad de implementar la ley 26.150, de Educación Sexual Integral, estableciendo políticas activas para la promoción y el fortalecimiento de la salud sexual y reproductiva de toda la población. Estas políticas deberán estar enmarcadas en los objetivos y alcances establecidos en las leyes 23.798, 25.673, 26.061, 26.075, 26.130, 26.150, 26.206, 26.485, 26.743 y 27.499, además de las leyes ya citadas en la presente ley. Deberán, además, capacitar sobre perspectiva de género y diversidad sexual a los y las docentes y a los y las profesionales y demás trabajadores y trabajadoras de la salud, a fin de brindar atención, contención y seguimiento adecuados a quienes soliciten realizar una interrupción voluntaria del embarazo en los términos de la presente ley, así como a los funcionarios públicos y las funcionarias públicas que actúen en dichos procesos.',    plainLanguageExplanation:
      'El Estado en todos sus niveles debe implementar la ESI (Ley 26.150) y capacitar a docentes y personal de salud en perspectiva de género. La IVE se inserta en un sistema más amplio de salud sexual y reproductiva.',
    practicalEffects: [
      'Las provincias no pueden resistir la implementación de la ESI',
      'Los hospitales deben capacitar a su personal de salud en perspectiva de género y diversidad sexual',
    ],
    examples: [],
    relatedArticles: ['art-27610-1', 'art-27610-19'],
    jurisprudence: [],
    regulations: [],
    keywords: ['ESI', 'educación sexual integral', 'perspectiva de género', 'capacitación', 'salud sexual'],
    order: 13,
    segments: [
      {
        id: 'seg-27610-13-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-13',
        articleNumber: '13',
        segmentType: 'PARAGRAPH',
        text:
          'El Estado nacional, las provincias, la Ciudad Autónoma de Buenos Aires y los municipios tienen la responsabilidad de implementar la ley 26.150, de Educación Sexual Integral...',
        plainExplanation:
          'El Estado debe implementar la ESI y capacitar a docentes y personal de salud en género y diversidad.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-14',
    lawId: 'ley-27610',
    number: '14',
    title: 'Modificación del artículo 85 del Código Penal',
    text:
      'Sustitúyese el artículo 85 del Código Penal de la Nación, por el siguiente: Artículo 85: El o la que causare un aborto será reprimido: 1. Con prisión de tres (3) a diez (10) años, si obrare sin consentimiento de la persona gestante. Esta pena podrá elevarse hasta quince (15) años si el hecho fuere seguido de la muerte de la persona gestante. 2. Con prisión de tres (3) meses a un (1) año, si obrare con consentimiento de la persona gestante, luego de la semana catorce (14) de gestación y siempre que no mediaren los supuestos previstos en el artículo 86.',    plainLanguageExplanation:
      'Reemplaza el art. 85 del Código Penal. Tipifica dos situaciones: (1) el aborto sin consentimiento de la persona gestante (3 a 10 años, hasta 15 si muere), y (2) el aborto con consentimiento pero después de la semana 14 y fuera de los casos del art. 86 (3 meses a 1 año). El aborto hasta semana 14 ya no es delito.',
    practicalEffects: [
      'El aborto voluntario hasta semana 14 ya no está penado para quien lo practica',
      'El aborto sin consentimiento sigue siendo un delito grave (3 a 10 años)',
      'Practicar un aborto con consentimiento después de la semana 14 sin encuadre legal: pena leve (3 meses a 1 año)',
    ],
    examples: [
      'Un médico que practica una IVE en semana 10 con consentimiento de la paciente no comete delito alguno',
    ],
    relatedArticles: ['art-27610-4', 'art-27610-16'],
    jurisprudence: [],
    regulations: [],
    keywords: ['Código Penal', 'art. 85', 'sin consentimiento', 'penas', 'despenalización'],
    order: 14,
    segments: [
      {
        id: 'seg-27610-14-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-14',
        articleNumber: '14',
        segmentType: 'PARAGRAPH',
        text:
          'Artículo 85: El o la que causare un aborto será reprimido: 1. Con prisión de tres (3) a diez (10) años, si obrare sin consentimiento de la persona gestante...',
        plainExplanation:
          'Reforma el art. 85 del CP: sin consentimiento = 3 a 10 años. Con consentimiento pero después de semana 14 fuera de las excepciones = 3 meses a 1 año.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [
      {
        id: 'amd-27610-art14-1',
        articleId: 'art-27610-14',
        modifyingLaw: 'Decreto 14/2021',
        modifyingDate: '2021-01-15',
        previousText: '',
        newText: '',
        description: 'El Decreto 14/2021 observó (vetó parcialmente) textos en negrita de los arts. 14 y 15 de la ley 27.610. Se mantuvo la reforma al art. 85 del CP.',
        createdAt: '2021-01-15T00:00:00.000Z',
      },
    ],
  },
  {
    id: 'art-27610-15',
    lawId: 'ley-27610',
    number: '15',
    title: 'Incorporación del artículo 85 bis al Código Penal',
    text:
      'Incorpórase como artículo 85 bis del Código Penal de la Nación, el siguiente: Artículo 85 bis: Será reprimido o reprimida con prisión de tres (3) meses a un (1) año e inhabilitación especial por el doble del tiempo de la condena, el funcionario público o la funcionaria pública o la autoridad del establecimiento de salud, profesional, efector o personal de salud que dilatare injustificadamente, obstaculizare o se negare, en contravención de la normativa vigente, a practicar un aborto en los casos legalmente autorizados.',    plainLanguageExplanation:
      'El nuevo art. 85 bis del Código Penal tipifica como delito que un médico, funcionario o establecimiento de salud dilate injustificadamente, obstaculice o niegue un aborto en los casos legales. La pena es 3 meses a 1 año de prisión más inhabilitación especial por el doble del tiempo de condena.',
    practicalEffects: [
      'El médico que demora o niega una IVE legal sin justificación puede ser procesado penalmente',
      'La inhabilitación especial impide ejercer la profesión durante el doble de la condena',
    ],
    examples: [
      'Un director de hospital que sistemáticamente obstaculiza las IVE en su institución puede ser denunciado y condenado bajo el art. 85 bis del CP',
    ],
    relatedArticles: ['art-27610-10', 'art-27610-14'],
    jurisprudence: [],
    regulations: [],
    keywords: ['art. 85 bis', 'obstaculización', 'denegación', 'pena', 'inhabilitación'],
    order: 15,
    segments: [
      {
        id: 'seg-27610-15-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-15',
        articleNumber: '15',
        segmentType: 'PARAGRAPH',
        text:
          'Artículo 85 bis: Será reprimido o reprimida con prisión de tres (3) meses a un (1) año e inhabilitación especial por el doble del tiempo de la condena, el funcionario público... que dilatare injustificadamente, obstaculizare o se negare, en contravención de la normativa vigente, a practicar un aborto en los casos legalmente autorizados.',
        plainExplanation:
          'Nuevo delito en el CP: el médico o funcionario que dilate o niegue una IVE legal puede ir preso y quedar inhabilitado.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-16',
    lawId: 'ley-27610',
    number: '16',
    title: 'Sustitución del artículo 86 del Código Penal',
    text:
      'Sustitúyese el artículo 86 del Código Penal de la Nación, por el siguiente: Artículo 86: No es delito el aborto realizado con consentimiento de la persona gestante hasta la semana catorce (14) inclusive del proceso gestacional. Fuera del plazo establecido en el párrafo anterior, no será punible el aborto practicado con el consentimiento de la persona gestante: 1. Si el embarazo fuere producto de una violación. En este caso, se debe garantizar la práctica con el requerimiento y la declaración jurada de la persona gestante ante el o la profesional o personal de salud interviniente. En los casos de niñas menores de trece (13) años de edad, la declaración jurada no será requerida. 2. Si estuviera en riesgo la vida o la salud integral de la persona gestante.',    plainLanguageExplanation:
      'Reemplaza el art. 86 del CP (el histórico "aborto no punible"). El nuevo texto establece que hasta semana 14 el aborto directamente no es delito. Después de la semana 14, no es punible en dos casos: (1) violación (con declaración jurada; sin ella para niñas menores de 13), y (2) riesgo para la vida o salud integral.',
    practicalEffects: [
      'Elimina el requisito de autorización judicial para el aborto en casos de violación',
      'La salud "integral" incluye salud mental: una mujer con riesgo psicológico grave también está amparada',
      'Reemplaza el sistema previo de "causales" por un sistema de plazos hasta semana 14',
    ],
    examples: [
      'Una mujer de 18 semanas embarazada por violación presenta declaración jurada ante su médico; no necesita ningún juicio ni autorización del juez',
    ],
    relatedArticles: ['art-27610-4', 'art-27610-14'],
    jurisprudence: [],
    regulations: [],
    keywords: ['art. 86', 'no es delito', 'semana 14', 'violación', 'salud integral', 'no punible'],
    order: 16,
    segments: [
      {
        id: 'seg-27610-16-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-16',
        articleNumber: '16',
        segmentType: 'PARAGRAPH',
        text:
          'Artículo 86: No es delito el aborto realizado con consentimiento de la persona gestante hasta la semana catorce (14) inclusive del proceso gestacional...',
        plainExplanation:
          'Hasta semana 14: no es delito. Después: no punible por violación o riesgo de vida/salud.',
        practicalExample:
          'Una mujer de 18 semanas embarazada por violación presenta declaración jurada al médico; no necesita ninguna orden judicial.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-17',
    lawId: 'ley-27610',
    number: '17',
    title: 'Sustitución del artículo 87 del Código Penal',
    text:
      'Sustitúyese el artículo 87 del Código Penal de la Nación, por el siguiente: Artículo 87: Será reprimido o reprimida con prisión de seis (6) meses a tres (3) años, el o la que con violencia causare un aborto sin haber tenido el propósito de causarlo, si el estado del embarazo de la persona gestante fuere notorio o le constare.',    plainLanguageExplanation:
      'Actualiza el art. 87 del CP (aborto preterintencional): quien cause un aborto con violencia pero sin intención de causarlo, sabiendo o debiendo saber del embarazo, recibe de 6 meses a 3 años. Solo actualiza el lenguaje para incluir "persona gestante".',
    practicalEffects: [
      'Sin cambio sustancial en el tipo penal: solo modernización del lenguaje con perspectiva de género',
    ],
    examples: [],
    relatedArticles: ['art-27610-14', 'art-27610-16'],
    jurisprudence: [],
    regulations: [],
    keywords: ['art. 87', 'aborto preterintencional', 'violencia', 'sin propósito'],
    order: 17,
    segments: [
      {
        id: 'seg-27610-17-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-17',
        articleNumber: '17',
        segmentType: 'PARAGRAPH',
        text:
          'Artículo 87: Será reprimido o reprimida con prisión de seis (6) meses a tres (3) años, el o la que con violencia causare un aborto sin haber tenido el propósito de causarlo...',
        plainExplanation:
          'Aborto involuntario causado con violencia: 6 meses a 3 años. Solo actualiza el lenguaje del CP.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-18',
    lawId: 'ley-27610',
    number: '18',
    title: 'Sustitución del artículo 88 del Código Penal',
    text:
      'Sustitúyese el artículo 88 del Código Penal de la Nación, por el siguiente: Artículo 88: Será reprimida con prisión de tres (3) meses a un (1) año, la persona gestante que, luego de la semana catorce (14) de gestación y siempre que no mediaren los supuestos previstos en el artículo 86, causare su propio aborto o consintiera que otro se lo causare. Podrá eximirse la pena cuando las circunstancias hicieren excusable la conducta. La tentativa de la persona gestante no es punible.',    plainLanguageExplanation:
      'Actualiza el art. 88 del CP. La persona gestante que interrumpa su propio embarazo después de la semana 14 y fuera de las causales del art. 86 puede recibir 3 meses a 1 año de prisión. Sin embargo: (1) el juez puede eximirla de pena si las circunstancias lo justifican, y (2) la tentativa (intento fallido) no es punible para la persona gestante.',
    practicalEffects: [
      'Hasta semana 14: la persona gestante no comete delito',
      'Desde semana 15 fuera de causales: posible pena, pero con posibilidad de exención',
      'La tentativa de la persona gestante nunca es punible',
    ],
    examples: [
      'Una mujer que intenta un aborto por sus medios a las 18 semanas no tiene posibilidad de ser condenada por la tentativa aunque fracase',
    ],
    relatedArticles: ['art-27610-14', 'art-27610-16'],
    jurisprudence: [],
    regulations: [],
    keywords: ['art. 88', 'persona gestante', 'tentativa no punible', 'exención de pena'],
    order: 18,
    segments: [
      {
        id: 'seg-27610-18-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-18',
        articleNumber: '18',
        segmentType: 'PARAGRAPH',
        text:
          'La tentativa de la persona gestante no es punible.',
        plainExplanation:
          'Si la persona gestante intenta interrumpir su embarazo y no lo logra, no puede ser condenada por ese intento.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-19',
    lawId: 'ley-27610',
    number: '19',
    title: 'Capacitación',
    text:
      'El personal de salud deberá capacitarse en los contenidos de esta ley y de la normativa complementaria y reglamentaria. A tal fin, el Ministerio de Salud de la Nación y los ministerios provinciales y de la Ciudad Autónoma de Buenos Aires implementarán los correspondientes programas de capacitación.',    plainLanguageExplanation:
      'El personal de salud tiene la obligación de formarse en los contenidos de la Ley IVE. Los ministerios de salud nacional y provinciales deben crear programas de capacitación.',
    practicalEffects: [
      'La ignorancia de la ley no exime al médico de sus obligaciones',
      'Los ministerios deben financiar y organizar la capacitación',
    ],
    examples: [],
    relatedArticles: ['art-27610-13'],
    jurisprudence: [],
    regulations: [],
    keywords: ['capacitación', 'personal de salud', 'ministerio de salud'],
    order: 19,
    segments: [
      {
        id: 'seg-27610-19-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-19',
        articleNumber: '19',
        segmentType: 'PARAGRAPH',
        text:
          'El personal de salud deberá capacitarse en los contenidos de esta ley y de la normativa complementaria y reglamentaria.',
        plainExplanation:
          'Todo el personal de salud debe formarse en esta ley. Los ministerios deben crear programas de capacitación.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-20',
    lawId: 'ley-27610',
    number: '20',
    title: 'Autoridad de aplicación',
    text:
      'La autoridad de aplicación de la presente ley será establecida por el Poder Ejecutivo nacional.',    plainLanguageExplanation:
      'El Poder Ejecutivo determina qué organismo aplica e implementa la ley. En la práctica, el Ministerio de Salud de la Nación es la autoridad de aplicación.',
    practicalEffects: [
      'El Ejecutivo puede reasignar la autoridad de aplicación sin necesidad de reformar la ley',
    ],
    examples: [],
    relatedArticles: [],
    jurisprudence: [],
    regulations: [],
    keywords: ['autoridad de aplicación', 'Poder Ejecutivo', 'Ministerio de Salud'],
    order: 20,
    segments: [
      {
        id: 'seg-27610-20-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-20',
        articleNumber: '20',
        segmentType: 'PARAGRAPH',
        text:
          'La autoridad de aplicación de la presente ley será establecida por el Poder Ejecutivo nacional.',
        plainExplanation:
          'El Ejecutivo decide quién aplica la ley. En la práctica, es el Ministerio de Salud.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-21',
    lawId: 'ley-27610',
    number: '21',
    title: 'Orden público',
    text:
      'Las disposiciones de la presente ley son de orden público y de aplicación obligatoria en todo el territorio de la República Argentina.',    plainLanguageExplanation:
      'La Ley IVE es de orden público: ninguna provincia, municipio, prepaga, hospital o juez puede apartarse de ella. Rige en todo el país, incluso en provincias que hubieran legislado de forma más restrictiva.',
    practicalEffects: [
      'Las ordenanzas o leyes provinciales que restrinjan el acceso a la IVE son inconstitucionales',
      'Ningún convenio o reglamento privado puede excluir las prestaciones de esta ley',
    ],
    examples: [
      'Una provincia que haya declarado "Provincia Pro-Vida" no puede impedir que sus hospitales públicos realicen IVE: la ley nacional prevalece',
    ],
    relatedArticles: ['art-27610-1', 'art-27610-12'],
    jurisprudence: [],
    regulations: [],
    keywords: ['orden público', 'aplicación nacional', 'provincias'],
    order: 21,
    segments: [
      {
        id: 'seg-27610-21-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-21',
        articleNumber: '21',
        segmentType: 'PARAGRAPH',
        text:
          'Las disposiciones de la presente ley son de orden público y de aplicación obligatoria en todo el territorio de la República Argentina.',
        plainExplanation:
          'Rige en todo el país. Ninguna provincia puede impedirla.',
        practicalExample:
          'Una provincia "pro-vida" no puede prohibir la IVE en sus hospitales: la ley nacional prevalece.',
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
  {
    id: 'art-27610-22',
    lawId: 'ley-27610',
    number: '22',
    title: 'Comuníquese',
    text:
      'Comuníquese al Poder Ejecutivo nacional.',    plainLanguageExplanation:
      'Cláusula de promulgación. Ordena comunicar la ley al Poder Ejecutivo para su promulgación y publicación en el Boletín Oficial.',
    practicalEffects: [],
    examples: [],
    relatedArticles: [],
    jurisprudence: [],
    regulations: [],
    keywords: ['promulgación'],
    order: 22,
    segments: [
      {
        id: 'seg-27610-22-0',
        lawId: 'ley-27610',
        articleId: 'art-27610-22',
        articleNumber: '22',
        segmentType: 'PARAGRAPH',
        text: 'Comuníquese al Poder Ejecutivo nacional.',
        plainExplanation: 'Cláusula de promulgación.',
        practicalExample: null,
        references: [],
        order: 0,
      },
    ],
    amendments: [],
  },
];
